...
% Plot the heading
figure;
plot(simout.heading_rad,'DisplayName','Path-Linearized Results','LineWidth',2)
grid on
hold on
plot(command,...
...
ylabel('Heading, [rad]')
set(gca,'FontSize',24)
fileName = 'HeadingComparison';
savePlot(gcf,folder,fileName);

% Plot the speed
figure;
plot(simout.speed_mPs,'DisplayName','Path-Linearized Results','LineWidth',2)
grid on
hold on
...
ylabel('Speed, [m/s]')
set(gca,'FontSize',24)
fileName = 'SpeedComparison';
savePlot(gcf,folder,fileName);

% Plot the 2D path
figure;
plot(squeeze(simout.xPosition_m.data),...
    squeeze(simout.yPosition_m.data),...
